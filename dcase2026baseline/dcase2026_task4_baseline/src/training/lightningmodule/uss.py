...
class USSLightning(BaseLightningModule):
    ...
    def _validation_breakdown(self, output_dict, target_dict):
        ...
        These are lightweight diagnostics, not replacements for CAPI-SDRi. They
        expose the 2026 failure modes that average loss can hide: zero-target
        scenes, same-class duplicates, and foreground count accuracy.
        """
        ...
        for b in range(class_index.shape[0]):
            ...
            same_class_flags.append(active_classes.unique().numel() < active_classes.numel() if active_classes.numel() > 1 else False)
        same_class = ref_count.new_tensor(same_class_flags, dtype=fg_active.dtype).bool()
        ...
        if "count_logits" in output_dict:
            ...
            count_target = ref_count.clamp(max=output_dict["count_logits"].shape[-1] - 1)
            diag.update({
                "valid_count_acc": (count_pred == count_target).float().mean(),
